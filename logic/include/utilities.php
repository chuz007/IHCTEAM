<?php    
     ...
     *gets files contents 
     * @param type $file
     * @param array $wrapper must be 2 values only, the opener tag and the close tag
     * @return obtained content string 
     */
    function renderFile($file,$wrapper=null){            
            ...
            try{
                ...
                if($wrapper!==null)
                {
                    $contents = $wrapper[0] . $contents . $wrapper[1];
                }
    ...
    function processPagePointer()
    {
        ...
        try{
            if(isset($_POST['pointer'])){                
                $pointerName=$_POST['pointer'];
            }elseif(isset($_GET['pointer'])){
                $pointerName=$_GET['pointer'];
            }else
                $pointerName="home";            
            ...
            {          
                ...
                if($hasMenu)
                {
                    $content .= $menuBuilder->createMenu();                    
                }
                $content.=$tempContent;
            }else
            {
    ...
    /*
     * Process a template by replacing the data object elements withing the template.
     * e.g If "Object->id = 2", this function will look for {id} and replace it for 2.
     * @Param1: $tmpl, is the template string where the data is going to be replaced.
     * @Param2: $dataObj, is the data element which is going to use for replacements
     * @returns: string with the template with data replaced
     */
    function template($tmpl,$dataObj)
    {       
        $sub = $tmpl;
        foreach($dataObj as $dataField => $value)
        {
            $sub = str_replace("{" . $dataField . "}", $value,$sub);
        }
        return $sub;
    }
    
    function getDivHeader($ID,$closeID=null,$class="",$title="")
    {
        $contents = '<div id="'. $ID .'" class="' . $class . '"><div style="float:left;" id="'. $ID . 'Content">' . $title . '</div>';
            if($closeID!=null)
                $contents .= '<div class="closeTag" onclick="closeDivByID(\''. $closeID .'\');"></div>';
        $contents .= '</div>';
        return $contents;
    }
